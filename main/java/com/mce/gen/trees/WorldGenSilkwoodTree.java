...
public class WorldGenSilkwoodTree extends WorldGenAbstractTree {
	...
	public WorldGenSilkwoodTree(boolean gen) {
		this(gen, 5, 0, 0, false);
	}

	public WorldGenSilkwoodTree(boolean gen, int height, int wood, int leaves,
			boolean vines) {
		super(gen);
		...
		if (y >= 1 && y + l + 1 <= 256) {
			...
			if (!flag) {
				...
				boolean isSoil = block2.canSustainPlant(world, x, y - 1, z,
						ForgeDirection.UP,
						(BlockSapling) mod_IDT.SilkwoodSapling);
				if (isSoil && y < 256 - l - 1) {
							...
							for (int k2 = z - l1; k2 <= z + l1; ++k2) {
								...
								if (Math.abs(j2) != l1 || Math.abs(l2) != l1
									...
									if (block1.isAir(world, i2, k1, k2)
											|| block1.isLeaves(world, i2, k1,
													k2)) {
										this.setBlockAndNotifyAdequately(world,
												i2, k1, k2,
												mod_IDT.SilkwoodLeaves,
												this.metaLeaves);
									}
								}
							}
					...
					for (k1 = 0; k1 < l; ++k1) {
						...
						if (block.isAir(world, x, y + k1, z)
							...
							this.setBlockAndNotifyAdequately(world, x, y + k1,
									z, mod_IDT.SilkwoodLog, this.metaWood);
						}
					}
