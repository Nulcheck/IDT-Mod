...
public class WorldGenWillowTree extends WorldGenAbstractTree {
	...
	public WorldGenWillowTree(boolean gen) {
		this(gen, 4, 0, 0, false);
	}

	public WorldGenWillowTree(boolean gen, int height, int wood, int leaves,
			boolean vines) {
		super(gen);
		...
		if (y >= 1 && y + l + 1 <= 256) {
			...
			if (!flag) {
				...
				boolean isSoil = block2
						.canSustainPlant(world, x, y - 1, z, ForgeDirection.UP,
								(BlockSapling) mod_IDT.WillowSapling);
					...
					for (k1 = 0; k1 < l; ++k1) {
						...
						if (block.isAir(world, x, y + k1, z)
							...
							this.setBlockAndNotifyAdequately(world, x, y + k1,
									z, mod_IDT.WillowLog, this.metaWood);
						}
					}
