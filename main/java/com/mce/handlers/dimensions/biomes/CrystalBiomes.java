...
public class CrystalBiomes extends BiomeGenBase {
	public static BiomeGenBase crystal;

	public CrystalBiomes(int id) {
		super(id);
	}

	static {
		crystal = (new CrystalBiome(mod_IDT.CrystalBiomeID).setBiomeName("Crystalline"));
	}

	public static void registerWithBiomeDictionary() {
		BiomeDictionary.registerBiomeType(crystal, Type.MAGICAL);
		...
		for (int l1 = 255; l1 >= 0; --l1) {
			...
			if (l1 <= 0 + random.nextInt(5)) {
				...
				if (block2 != null && block2.getMaterial() != Material.air) {
					if (block2 == mod_IDT.CrystalRock) {
						if (k == -1) {
							if (l <= 0) {
								block = null;
								b0 = 0;
								block1 = mod_IDT.CrystalRock;
							} else if (l1 >= 59 && l1 <= 64) {
								block = this.topBlock;
							...
							if (l1 >= 62) {
								...
								block = null;
								block1 = mod_IDT.CrystalRock;
								replacableBlock[i2] = mod_IDT.CrystalRock; // Gravel
							} else {
