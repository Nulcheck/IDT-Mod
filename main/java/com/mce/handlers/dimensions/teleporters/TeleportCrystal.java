...
public class TeleportCrystal extends Teleporter {
	...
	public TeleportCrystal(WorldServer server) {
		super(server);
			...
			for (int l = -2; l <= 2; ++l) {
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						int i2 = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						this.worldServerInstance.setBlock(k1, l1, i2, flag ? mod_IDT.CrystalGlass : Blocks.air);
					}
				}
			}
	...
	public boolean placeInExistingPortal(Entity entity, double x, double y, double z, float f) {
		...
		if (this.destinationCoordinateCache.containsItem(j1)) {
			TeleportCrystal.PortalPosition portalposition = (TeleportCrystal.PortalPosition) this.destinationCoordinateCache
					.getValueByKey(j1);
			d3 = 0.0D;
			...
			for (l3 = l - short1; l3 <= l + short1; ++l3) {
				...
				for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
					...
					for (int i2 = this.worldServerInstance.getActualHeight() - 1; i2 >= 0; --i2) {
						if (this.worldServerInstance.getBlock(l3, i2, l1) == mod_IDT.CrystalPortal) {
							while (this.worldServerInstance.getBlock(l3, i2 - 1, l1) == mod_IDT.CrystalPortal) {
								--i2;
							}
		...
		if (d3 >= 0.0D) {
			if (flag) {
				this.destinationCoordinateCache.add(j1,
						new TeleportCrystal.PortalPosition(i, j, k, this.worldServerInstance.getTotalWorldTime()));
			...
			if (this.worldServerInstance.getBlock(i - 1, j, k) == mod_IDT.CrystalPortal) {
				i4 = 2;
			}
			...
			if (this.worldServerInstance.getBlock(i, j, k - 1) == mod_IDT.CrystalPortal) {
				i4 = 3;
			}
			...
			for (i3 = -1; i3 <= 1; ++i3) {
				for (j3 = 1; j3 < 3; ++j3) {
					for (k3 = -1; k3 < 3; ++k3) {
						...
						j4 = k2 + (j3 - 1) * l2 - i3 * l5;
						flag = k3 < 0;
						this.worldServerInstance.setBlock(l3, i4, j4, flag ? mod_IDT.CrystalGlass : Blocks.air);
					}
				}
			}
		}
			...
			for (j3 = 0; j3 < 4; ++j3) {
				for (k3 = -1; k3 < 4; ++k3) {
					...
					this.worldServerInstance.setBlock(l3, i4, j4,
							(Block) (flag ? mod_IDT.CrystalGlass : mod_IDT.CrystalPortal), 0, 2);
				}
			}
