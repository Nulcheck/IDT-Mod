...
public class ChunkProviderFrost implements IChunkProvider {
	/** RNG. */
	private Random rand;
	...
	public ChunkProviderFrost(World world, long seed, boolean features) {
		this.worldObj = world;
					...
					for (int l2 = 0; l2 < 8; ++l2) {
						...
						for (int i3 = 0; i3 < 4; ++i3) {
							...
							for (int k3 = 0; k3 < 4; ++k3) {
								if ((d15 += d16) > 0.0D) {
									block[j3 += short1] = mod_IDT.FrostStone;
								} else if (k2 * 8 + l2 < b0) {
	...
	public void populate(IChunkProvider chunk, int x, int z) {
		...
		if (biomegenbase != BiomeGenBase.desert && biomegenbase != BiomeGenBase.desertHills && !flag
				&& this.rand.nextInt(4) == 0 && TerrainGen.populate(chunk, worldObj, rand, x, z, flag, LAKE)) {
