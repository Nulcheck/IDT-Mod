...
public class FrozenFire extends BlockFire {
	@Deprecated
	...
	public void onBlockAdded(World world, int x, int y, int z) {
		if (world.provider.dimensionId > 0 || !((FrozenPortal) mod_IDT.FrozenPortal).getPortalSize(world, x, y, z)) {
			if (!World.doesBlockHaveSolidTopSurface(world, x, y - 1, z) && !this.canNeighborBurn(world, x, y, z)) {
				world.setBlockToAir(x, y, z);
			} else {
	...
	public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
		...
		if (!World.doesBlockHaveSolidTopSurface(world, x, y - 1, z)
				&& !mod_IDT.FrozenFire.canCatchFire(world, x, y - 1, z, UP)) {
			if (mod_IDT.FrozenFire.canCatchFire(world, x - 1, y, z, EAST)) {
				for (l = 0; l < 2; ++l) {
			...
			if (mod_IDT.FrozenFire.canCatchFire(world, x + 1, y, z, WEST)) {
				for (l = 0; l < 2; ++l) {
			...
			if (mod_IDT.FrozenFire.canCatchFire(world, x, y, z - 1, SOUTH)) {
				for (l = 0; l < 2; ++l) {
			...
			if (mod_IDT.FrozenFire.canCatchFire(world, x, y, z + 1, NORTH)) {
				for (l = 0; l < 2; ++l) {
					f = (float) x + rand.nextFloat();
					f1 = (float) y + rand.nextFloat();
			...
			if (mod_IDT.FrozenFire.canCatchFire(world, x, y + 1, z, DOWN)) {
				for (l = 0; l < 2; ++l) {
					f = (float) x + rand.nextFloat();
	...
	public void registerBlockIcons(IIconRegister icon) {
		this.field_149850_M = new IIcon[] { icon.registerIcon("mod_IDT:snow_fire_layer_0"),
				icon.registerIcon("mod_IDT:snow_fire_layer_0") };
	}
