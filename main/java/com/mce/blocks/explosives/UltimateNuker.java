...
public class UltimateNuker extends BlockContainer {
	IIcon blockIcon;

	public UltimateNuker() {
		super(Material.tnt);
	}
	...
	public void onBlockDestroyedByExplosion(World world, int x, int y, int z, Explosion exp) {
		if (!world.isRemote) {
			EntityUN un = new EntityUN(world, (double) ((float) x + 0.5F), (double) ((float) y + 0.5F),
					(double) ((float) z + 0.5F), exp.getExplosivePlacedBy());
			un.fuse = world.rand.nextInt(un.fuse / 4) + un.fuse / 8;
			world.spawnEntityInWorld(un);
		}
	}
	...
	public void primeTnt(World world, int x, int y, int z, int meta, EntityLivingBase entity) {
		if (!world.isRemote) {
			if ((meta & 1) == 1) {
				...
				world.spawnEntityInWorld(un);
				world.playSoundAtEntity(un, "random.fuse", 1.0F, 1.0F);
			}
		}
	}

	public TileEntity createNewTileEntity(World world, int meta) {
		return new TileEntityUN();
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
		...
		if (player.getCurrentEquippedItem() != null
			...
			System.out.println("An error has occurred! Error ID: IDT-304d");
