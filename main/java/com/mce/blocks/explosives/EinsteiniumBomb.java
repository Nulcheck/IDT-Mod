...
public class EinsteiniumBomb extends BlockContainer {
	IIcon blockIcon;

	public EinsteiniumBomb() {
		super(Material.tnt);
	}
	...
	public void onBlockDestroyedByExplosion(World world, int x, int y, int z, Explosion exp) {
		if (!world.isRemote) {
			EntityEB eb = new EntityEB(world, (double) ((float) x + 0.5F), (double) ((float) y + 0.5F),
					(double) ((float) z + 0.5F), exp.getExplosivePlacedBy());
			eb.fuse = world.rand.nextInt(eb.fuse / 4) + eb.fuse / 8;
	...
	public void primeTnt(World world, int x, int y, int z, int meta, EntityLivingBase entity) {
		if (!world.isRemote) {
			if ((meta & 1) == 1) {
				...
				world.spawnEntityInWorld(eb);
				world.playSoundAtEntity(eb, "random.fuse", 1.0F, 1.0F);
			}
		}
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
		...
		if (player.getCurrentEquippedItem() != null
			...
			System.out.println("An error has occurred! Error ID: IDT-304b");
