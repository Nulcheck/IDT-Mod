...
public class DarkOakChest extends BlockContainer {
	...
	public void onBlockPlacedBy(World world, int x, int y, int z,
		...
		if (stack.hasDisplayName()) {
			((TileEntityDarkOakChest) world.getTileEntity(x, y, z))
					.func_145976_a(stack.getDisplayName());
		}
	}
	...
	public boolean canPlaceBlockAt(World world, int x, int y, int z) {
		...
		return l > 1 ? false : (this.func_149952_n(world, x - 1, y, z) ? false
				: (this.func_149952_n(world, x + 1, y, z) ? false : (this
						.func_149952_n(world, x, y, z - 1) ? false : !this
						.func_149952_n(world, x, y, z + 1))));
	}

	private boolean func_149952_n(World world, int x, int y, int z) {
		return world.getBlock(x, y, z) != this ? false : (world.getBlock(x - 1,
				y, z) == this ? true
				: (world.getBlock(x + 1, y, z) == this ? true : (world
						.getBlock(x, y, z - 1) == this ? true : world.getBlock(
						x, y, z + 1) == this)));
	}
	...
	public void onNeighborBlockChange(World world, int x, int y, int z,
		...
		TileEntityDarkOakChest tileentitychest = (TileEntityDarkOakChest) world
				.getTileEntity(x, y, z);
	...
	public boolean onBlockActivated(World world, int x, int y, int z,
			EntityPlayer player, int meta, float open, float close,
			float p_149727_9_) {
		if (world.isRemote) {
			return true;
		} else {
	...
	public IInventory func_149951_m(World world, int x, int y, int z) {
		Object object = (TileEntityDarkOakChest) world.getTileEntity(x, y, z);
		...
		if (object == null) {
			...
			return null;
				...
				&& (world.isSideSolid(x - 1, y + 1, z, DOWN) || func_149953_o(
						world, x - 1, y, z))) {
			return null;
		} else if (world.getBlock(x + 1, y, z) == this
				&& (world.isSideSolid(x + 1, y + 1, z, DOWN) || func_149953_o(
			...
			return null;
				...
				&& (world.isSideSolid(x, y + 1, z - 1, DOWN) || func_149953_o(
						world, x, y, z - 1))) {
			return null;
		} else if (world.getBlock(x, y, z + 1) == this
				&& (world.isSideSolid(x, y + 1, z + 1, DOWN) || func_149953_o(
			...
			if (world.getBlock(x - 1, y, z) == this) {
				object = new InventoryLargeChest(
						"container.darkOakChestDouble",
						(TileEntityDarkOakChest) world.getTileEntity(x - 1, y,
								z), (IInventory) object);
			}

			if (world.getBlock(x + 1, y, z) == this) {
				object = new InventoryLargeChest(
						"container.darkOakChestDouble", (IInventory) object,
						(TileEntityDarkOakChest) world.getTileEntity(x + 1, y,
								z));
			}
			...
			if (world.getBlock(x, y, z - 1) == this) {
				object = new InventoryLargeChest(
						"container.darkOakChestDouble",
						(TileEntityDarkOakChest) world.getTileEntity(x, y,
								z - 1), (IInventory) object);
			}
	...
	public TileEntity createNewTileEntity(World world, int meta) {
		TileEntityDarkOakChest tileentitychest = new TileEntityDarkOakChest();
	...
	public int isProvidingWeakPower(IBlockAccess blockacc, int x, int y, int z,
			int meta) {
		if (!this.canProvidePower()) {
			return 0;
		} else {
			int i1 = ((TileEntityDarkOakChest) blockacc.getTileEntity(x, y, z)).numPlayersUsing;
			return MathHelper.clamp_int(i1, 0, 15);
		}
	}

	public int isProvidingStrongPower(IBlockAccess blockacc, int x, int y,
			int z, int meta) {
		return meta == 1 ? this.isProvidingWeakPower(blockacc, x, y, z, meta)
				: 0;
	}
	...
	private static boolean func_149953_o(World world, int x, int y, int z) {
		Iterator iterator = world.getEntitiesWithinAABB(
				EntityOcelot.class,
				AxisAlignedBB.getBoundingBox((double) x, (double) (y + 1),
						(double) z, (double) (x + 1), (double) (y + 2),
						(double) (z + 1))).iterator();
	...
	public int getComparatorInputOverride(World world, int x, int y, int z,
			int p_149736_5_) {
		return Container.calcRedstoneFromInventory(this.func_149951_m(world, x,
				y, z));
	}

	@SideOnly(Side.CLIENT)
	public void registerBlockIcons(IIconRegister p_149651_1_) {
		this.blockIcon = p_149651_1_.registerIcon("planks_big_oak");
	}

	public void unifyAdjacentChests(World worldObj, int xCoord, int yCoord,
			int zCoord) {
