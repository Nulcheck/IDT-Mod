...
public class DarkMatterChest extends BlockContainer {
	private final Random Rando = new Random();
	public final int field_149956_a;

	public DarkMatterChest(int meta) {
		super(Material.wood);
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack stack) {
		...
		if (stack.hasDisplayName()) {
			((TileEntityDarkMatterChest) world.getTileEntity(x, y, z)).func_145976_a(stack.getDisplayName());
		}
	}
	...
	public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
		...
		TileEntityDarkMatterChest tileentitychest = (TileEntityDarkMatterChest) world.getTileEntity(x, y, z);
	...
	public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
		...
		if (tileentitychest != null) {
			for (int i1 = 0; i1 < tileentitychest.getSizeInventory(); ++i1) {
				...
				if (itemstack != null) {
					float f = this.Rando.nextFloat() * 0.8F + 0.1F;
					float f1 = this.Rando.nextFloat() * 0.8F + 0.1F;
					EntityItem entityitem;

					for (float f2 = this.Rando.nextFloat() * 0.8F + 0.1F; itemstack.stackSize > 0; world
							.spawnEntityInWorld(entityitem)) {
						int j1 = this.Rando.nextInt(21) + 10;
						...
						float f3 = 0.05F;
						entityitem.motionX = (double) ((float) this.Rando.nextGaussian() * f3);
						entityitem.motionY = (double) ((float) this.Rando.nextGaussian() * f3 + 0.2F);
						entityitem.motionZ = (double) ((float) this.Rando.nextGaussian() * f3);
	...
	/*public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
		...
		if(tile instanceof TileEntityDarkMatterChest){
			TileEntityDarkMatterChest te = (TileEntityDarkMatterChest)tile;
			
			ItemStack stack = new ItemStack(Item.getItemFromBlock(this));
			NBTTagCompound tag = new NBTTagCompound();
			NBTTagCompound tag2 = new NBTTagCompound();
			tag.setTag("BlockEntityTag", ((TileEntityDarkMatterChest)tile).saveToNBT(tag2));
			stack.setTagCompound(tag);
			
			if(te.hasCustomInventoryName()){
				stack.setStackDisplayName(te.getInventoryName());
				te.func_145976_a("");
			}
			//spawnAsEntity
		}
	 ...
	 * EntityPlayer player) { if (world.getTileEntity(x, y, z) instanceof
	 * TileEntityDarkMatterChest){ TileEntityDarkMatterChest te =
	 * (TileEntityDarkMatterChest)world.getTileEntity(x, y, z); } }
	 */
	...
	public IInventory func_149951_m(World world, int x, int y, int z) {
		Object object = (TileEntityDarkMatterChest) world.getTileEntity(x, y, z);
		...
		if (object == null) {
			...
			if (world.getBlock(x - 1, y, z) == this) {
				object = new InventoryLargeChest("container.dmChestDouble",
						(TileEntityDarkMatterChest) world.getTileEntity(x - 1, y, z), (IInventory) object);
			}

			if (world.getBlock(x + 1, y, z) == this) {
				object = new InventoryLargeChest("container.dmChestDouble", (IInventory) object,
						(TileEntityDarkMatterChest) world.getTileEntity(x + 1, y, z));
			}

			if (world.getBlock(x, y, z - 1) == this) {
				object = new InventoryLargeChest("container.dmChestDouble",
						(TileEntityDarkMatterChest) world.getTileEntity(x, y, z - 1), (IInventory) object);
			}

			if (world.getBlock(x, y, z + 1) == this) {
				object = new InventoryLargeChest("container.dmChestDouble", (IInventory) object,
						(TileEntityDarkMatterChest) world.getTileEntity(x, y, z + 1));
			}
	...
	public TileEntity createNewTileEntity(World world, int meta) {
		TileEntityDarkMatterChest tileentitychest = new TileEntityDarkMatterChest();
	...
	public int isProvidingWeakPower(IBlockAccess blockacc, int x, int y, int z, int meta) {
		if (!this.canProvidePower()) {
			...
			int i1 = ((TileEntityDarkMatterChest) blockacc.getTileEntity(x, y, z)).numPlayersUsing;
	...
	public void registerBlockIcons(IIconRegister p_149651_1_) {
		this.blockIcon = p_149651_1_.registerIcon("mod_IDT:dark_matter");
	}
