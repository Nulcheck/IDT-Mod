...
public class SlotOutWelder extends Slot {
	private EntityPlayer thePlayer;
	private int number;
	private static final String __OBFID = "CL_00001749";

	public SlotOutWelder(EntityPlayer player, IInventory inv, int x, int y, int z) {
		super(inv, x, y, z);
	...
	protected void onCrafting(ItemStack stack) {
		...
		if (!this.thePlayer.worldObj.isRemote) {
			int i = this.number;
			float f = ((WelderRecipes) WelderRecipes.welding()).getXp(stack);
			int j;
