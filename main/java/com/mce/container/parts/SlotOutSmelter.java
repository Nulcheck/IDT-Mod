...
public class SlotOutSmelter extends Slot {
	private EntityPlayer thePlayer;
	private int number;
	private static final String __OBFID = "CL_00001749";

	public SlotOutSmelter(EntityPlayer player, IInventory inv, int x, int y, int z) {
		super(inv, x, y, z);
	...
	protected void onCrafting(ItemStack stack) {
		...
		if (stack.getItem() == mod_IDT.SteelIngot) {
			this.thePlayer.addStat(AchRegistry.steelIngot, 1);
		}

		if (stack.getItem() == mod_IDT.TitaniumIngot) {
			this.thePlayer.addStat(AchRegistry.titaniumIngot, 1);
		}
	}
}